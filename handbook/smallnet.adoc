...
Once we have connectivity, we can of course send messages from the shell:
...
We have already seen in <<Getting started>> and <<Protocol numbers>> on how to send messages using the UnetSocket API from the shell, as well as from external applications. Hence we won't dwell on it here.
