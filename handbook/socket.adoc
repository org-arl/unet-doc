...
----
<1> Looking up an agent based on a services it provides is recommended, rather than specify the agent by name. We will explore services in more detail in <<Understanding UnetStack services>>. However, if you wished to reference an agent by name, you could have done that as: `def phy = sock.agent('phy')`
