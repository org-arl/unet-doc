...
While you can access a lot of functionality via parameters and commands, you'll need an understanding of the underlying messaging system for advanced usage of the stack. All agents support a set of messages that expose their functionality. In fact, all parameters and commands are implemented by exchanging messages between the shell agent and other agents. In this section, we'll see how messaging between agents works.

Typically, we would want to send a request message to an agent and get a response message back. This can be accomplished with the `request` call (or the equivalent alias `<<`) on the agent:
