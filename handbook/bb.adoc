...
Agents supporting the baseband service provide a set of messages to for arbitrary signal transmission and recording:
...
TIP: You can generate other frequency signals using the `cw()` function available in the shell. You can also save and load signals from text files using the `save` and `load` commands. For information on all these commands/functions, simply type `help cw`, `help save` or `help load`.
...
If you don't have access to another computer run unet audio on, we can easily demonstrate the above behavior with a single unet audio SDOAM by simply enabling the full-duplex mode (and hence using the same computer for transmission and reception simultaneously), as we did in <<Physical service>>:
...
If you enable `basebandRx`, a recording will be triggered everytime the preamble is detected:
...
----
<1> We are requesting 10 ms recording beyond the end of the preamble.
...
As you record more signals, they are appended to the same file. If you restart unet audio, this file will be renumbered to `signals-1.txt`, as the logs are rotated.
...
In[2]:   # connect to the unet audio SDOAM
         sock = unetpy.UnetSocket('localhost', 1100)
         gw = sock.getGateway()

In[3]:   # lookup the agent providing baseband service
         bb = gw.agentForService('org.arl.unet.Services.BASEBAND')
         bb.name
Out[3]:  'phy'

In[4]:   # define the messages we plan to use
         TxBasebandSignalReq = unetpy.MessageClass('org.arl.unet.bb.TxBasebandSignalReq')
         RecordBasebandSignalReq = unetpy.MessageClass('org.arl.unet.bb.RecordBasebandSignalReq')

In[5]:   # transmit preamble 3 -- you should be able to hear it
         bb << TxBasebandSignalReq(preamble=3)
Out[5]:  AGREE

In[6]:   # discard old notifications to get ready for a recording
         gw.flush()

In[7]:   # request a recording
         bb << RecordBasebandSignalReq()
Out[7]:  AGREE

In[8]:   # obtain the recording notification and check that it's of the correct type
         ntf = gw.receive(timeout=5000)
         ntf, ntf.__clazz__
Out[8]:  (INFORM, 'org.arl.unet.bb.RxBasebandSignalNtf')

In[9]:   # close the connection
         sock.close()

In[10]:  len(ntf.signal)
Out[10]: 131072

In[11]:  # plot the first 10000 baseband samples (real/in-phase components only)
         plt.plot(ntf.signal[:10000].real, fs=ntf.fs)
Out[11]:
----
image::sig1.png[width=600]
