...
`modem.powerLevel = [-42, -42, -42]`:: Transmit power level (dB re `refPowerLevel`).

In addition to the above indexed properties, several other properties control the modem behavior:

`modem.signalPowerLevel = -42`:: Transmit power level (dB re `refPowerLevel`) for baseband signals.
